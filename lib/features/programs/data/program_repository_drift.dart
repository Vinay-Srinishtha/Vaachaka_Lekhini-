...
class ProgramRepositoryDrift implements ProgramRepository {
  ...
  Future<PracticeSession> finishSession(String sessionId) async {
    ...
    await (_db.update(_db.sessions)..where((t) => t.id.equals(sessionId))).write(
      SessionsCompanion(endedAt: Value(now), updatedAt: Value(now)),
    );
    final row = await (_db.select(_db.sessions)..where((t) => t.id.equals(sessionId))).getSingle();
    final session = _toSession(row);
    // Roll the count into the program's totals.
    final programRow = await (_db.select(_db.programs)..where((t) => t.id.equals(session.programId))).getSingleOrNull();
    if (programRow != null) {
      await (_db.update(_db.programs)..where((t) => t.id.equals(session.programId))).write(
        ProgramsCompanion(
  ...
  Future<int> longestStreak(String programId) async {
    final rows = await (_db.select(_db.sessions)
        ...
        .get();
    if (rows.isEmpty) return 0;
    final days = rows.map((r) => _startOfDay(r.startedAt)).toSet().toList()..sort();
    var longest = 1;
    var current = 1;
