...
class ProgramRepositoryDrift implements ProgramRepository {
  ...
  Future<Program> create({
    ...
    final program = Program(
      ...
      dailyTarget: ProgramRepository.computeDailyTarget(targetWritings, targetDays),
      startedAt: now,
      createdAt: now,
      updatedAt: now,
    );
  ...
  Future<void> update(Program program) async {
    final now = DateTime.now();
    await (_db.update(_db.programs)..where((t) => t.id.equals(program.id))).write(
      ProgramsCompanion(
    ...
    await _outbox.enqueue('programs.upsert', _programPayload(program.copyWith(updatedAt: now)));
  }
  ...
  Future<PracticeSession> finishSession(String sessionId) async {
    ...
    final programRow = await (_db.select(_db.programs)
          ..where((t) => t.id.equals(session.programId)))
        .getSingleOrNull();
    if (programRow != null) {
      final isHandwriting = session.modality == SessionModality.handwriting;
      final updatedProgram = isHandwriting
          ? ProgramsCompanion(
              totalWritings: Value(programRow.totalWritings + session.countAdded),
      ...
      await (_db.update(_db.programs)..where((t) => t.id.equals(session.programId)))
          .write(updatedProgram);
      ...
      final currentProgramRow = await (_db.select(_db.programs)
            ..where((t) => t.id.equals(session.programId)))
          .getSingle();
      final newLongest = streak > currentProgramRow.longestStreak ? streak : currentProgramRow.longestStreak;
      await (_db.update(_db.programs)..where((t) => t.id.equals(session.programId)))
      ...
      final updatedProgramRow = await (_db.select(_db.programs)
            ..where((t) => t.id.equals(session.programId)))
          .getSingle();
      await _outbox.enqueue('programs.upsert', _programPayload(_toProgram(updatedProgramRow)));
    }
  ...
  Future<Map<DateTime, int>> sessionCountsByDay({
    ...
    final sessions = await (_db.select(_db.sessions)
          ..where((t) =>
              ...
              t.startedAt.isSmallerThanValue(to.add(const Duration(days: 1)))))
        .get();
  ...
  Future<DailySummary> dailySummary(String programId, DateTime day) async {
    ...
    final usedHandwriting = rows.any((r) => r.modality == SessionModality.handwriting.name);
    return DailySummary(
      day: start,
  ...
  Future<int> currentStreak(String programId) async {
    ...
    final row = await (_db.select(_db.programs)..where((t) => t.id.equals(programId)))
        .getSingleOrNull();
  ...
  Future<Program?> mostRecentlyActive(String memberId) async {
    final session = await (_db.select(_db.sessions).join([
      innerJoin(_db.programs, _db.programs.id.equalsExp(_db.sessions.programId)),
    ])
    ...
    final p = await (_db.select(_db.programs)
          ..where((t) => t.memberId.equals(memberId))
          ..orderBy([(t) => OrderingTerm.desc(t.updatedAt)])
          ..limit(1))
        .getSingleOrNull();
...
extension _SessionCopy on PracticeSession {
  PracticeSession copyWith({int? durationSec, DateTime? endedAt}) => PracticeSession(
        id: id,
        programId: programId,
        memberId: memberId,
        startedAt: startedAt,
