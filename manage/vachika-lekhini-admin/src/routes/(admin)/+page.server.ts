...
export const load: PageServerLoad = async () => {
	...
	const [
		...
		deviceCount,
		chantsToday,
		totalChantsAgg, totalWritingsAgg,
		...
		prisma.program.aggregate({ _sum: { totalChants: true } }),
		prisma.program.aggregate({ _sum: { totalWritings: true } }),
	...
	return {
		stats: {
			...
			totalChants:   totalChantsAgg._sum.totalChants ?? 0,
			totalWritings: totalWritingsAgg._sum.totalWritings ?? 0,
