...
<div class="space-y-4">
	...
	{#if data.reports.length === 0}
		<p class="text-gray-500 text-sm py-8 text-center">No feedback submitted yet.</p>
	{:else}
		<div class="space-y-3">
								...
								{#each ['open', 'resolved', 'dismissed'] as s}
									{#if s !== r.status}
										<form method="POST" action="?/setStatus" use:enhance>
											...
											<button class="text-xs px-3 py-1 rounded border border-gray-200 hover:bg-gray-50">
												Mark {s}
											</button>
										</form>
									{/if}
