...
>
	{#if editingRole}
		...
		>
			...
			<!-- Hidden inputs: one per selected permission -->
			{#each [...editPerms] as p}
					...
					<div class="rounded-xl border border-gray-200 overflow-hidden">
						...
						>
							...
							>
								...
								{:else if state === 'some'}
									<span class="w-1.5 h-0.5 bg-brand-600 rounded-full"></span>
								{/if}
							</span>
						...
						<div class="flex flex-wrap gap-2 px-4 py-3 bg-white">
							{#each group.permissions as perm}
								{@const active = editPerms.has(perm.key)}
								<button
									type="button"
									onclick={() => togglePerm(perm.key)}
									class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium border transition-all
										{active
											? 'bg-brand-600 text-white border-brand-600 shadow-sm'
											: 'bg-white text-gray-600 border-gray-200 hover:border-brand-400 hover:text-brand-700'}"
								>
									{#if active}
										<svg class="w-3 h-3" fill="none" viewBox="0 0 10 8">
											<path d="M1 4l3 3 5-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
										</svg>
									{/if}
									{perm.label}
			...
			<div class="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between gap-4">
				<div class="text-sm text-gray-500">
					<span class="font-semibold text-gray-800">{editPerms.size}</span>
					of {data.allPermissions.length} permissions selected
				</div>
				<div class="flex items-center gap-3">
					<button type="button" class="btn-secondary" onclick={closeEdit}>Cancel</button>
					<button type="submit" class="btn-primary" disabled={submitting}>
						{#if submitting}Saving…{:else}<ShieldCheck size={14} /> Save Changes{/if}
					</button>
				</div>
			</div>
		</form>
